...
> - **دور الضيف يعمل على Windows فقط.** على macOS يرفض الإقلاع ويقول السبب: فحص مالك الاتصال غير
>   منفَّذ هناك، وبدونه يمرّ كل ما على الجهاز لا متصفح العمل وحده. **المضيف على macOS يعمل.**
>   التفصيل في [macos-port.md](macos-port.md).
> - **لا شهادة توقيع كود** ([ADR-0011](decisions/0011-no-code-signing-certificate.md))، فسيحذّر
>   SmartScreen على Windows وGatekeeper على macOS. الدليل يشرح كيف تتجاوز التحذير بوعي.
...
## الجزء الثاني: الجهازان (الضيف: Windows فقط)
