# خطة تنفيذ RouteBridge — الإصدار الأول (MVP)

## 1. السياق

المشروع يبدأ من الصفر: مجلد `/Users/moshaker/project` لا يحتوي إلا على `RouteBridge.pdf` (وثيقة تعريف المنتج وخطة الإصدارات، 16 صفحة). المطلوب خطة تنفيذ كاملة للإصدار الأول كما حددته الوثيقة في الأقسام 6 و10 إلى 17.
...
| لوحة الإدارة | لا واجهة ويب. CLI (`manage.py`) + OpenAPI التفاعلي | الوثيقة تؤجلها للإصدار الثاني |
...
│   ├── RouteBridge.sln
│   ├── src/
│   │   ├── RouteBridge.Core/         # النماذج، آلة حالة الجلسة، AllowlistMatcher، IpRangePolicy، رسائل البروتوكول (بلا تبعيات)
│   │   ├── RouteBridge.Tunnel/       # TlsTunnelFactory، AuthHandshake، CandidateGatherer، CandidateDialer، TunnelListener، Mux
│   │   ├── RouteBridge.Proxy/        # ConnectProxyServer (جهة المستخدم)، ProbePage، OwnerPidChecker
│   │   ├── RouteBridge.Egress/       # EgressPolicy، OpenHandler (جهة المضيف)، ByteCounter، DomainCollector
│   │   ├── RouteBridge.Browser/      # BrowserLocator، PolicyDetector، BrowserLauncher (Job Object)، GracefulCloser
│   │   ├── RouteBridge.Infrastructure/ # ApiClient، ControlChannel (WS)، DpapiSecretStore، DeviceInfo، Serilog
│   │   └── RouteBridge.App/          # WPF + MVVM + Tray + Toasts + DI (Generic Host)
│   ├── tests/                        # RouteBridge.Core.Tests، Tunnel.Tests، Proxy.Tests، Egress.Tests، Browser.Tests
│   ├── tools/RouteBridge.Spike/      # أدوات Console للنموذج التقني (المرحلة 0)
...
3. لكل اتصال TCP ناجح: مصافحة TLS (10 ثوانٍ) — المتصل يثبّت بصمة شهادة الطرف الآخر ويتجاهل أخطاء السلسلة، `TargetHost` ثابت `routebridge`، بلا فحص إبطال — ثم مصادقة داخل TLS (5 ثوانٍ). **المستخدم يتكلم أولًا دائمًا** أيًا كان من اتصل.
...
- **صفحة الفحص:** المتصفح يُفتح على `http://check.routebridge/`؛ الـ Proxy يعترضها ويرد بصفحة «النفق نشط. المواقع سترى: <IP المضيف العام>». إن لم يصل هذا الطلب خلال 10 ثوانٍ فالمتصفح لا يستخدم الـ Proxy → إنهاء الجلسة بسبب `browser_not_proxied`.
...
| `Infrastructure` | `ApiClient` (تجديد التوكن الشفاف)، `ControlChannel` (`ClientWebSocket` + إعادة اتصال بتراجع أسّي + نبض + إعادة إعلان «متاح» + احترام Proxy النظام)، `DpapiSecretStore` (التوكنات وسر الجهاز)، `DeviceInfoProvider`، `FirewallRuleChecker`، `VpnAdapterDetector`، Serilog إلى `%LOCALAPPDATA%\RouteBridge\logs` (بلا أي حمولة إطارات) |
...
```
--user-data-dir="%LocalAppData%\RouteBridge\BrowserProfile"
...
--new-window "http://check.routebridge/"
```
...
| E2E يدوي | جهازا Windows (10 و11) على شبكتين؛ `docs/acceptance-checklist.md` يغطي معايير النجاح الـ 18: ifconfig.me يعرض IP المضيف في متصفح العمل وIP المستخدم في متصفحه العادي، Teams/Outlook لا تتأثر، القطع من الطرفين، الانتهاء التلقائي، عدم مرور بيانات عبر FastAPI (iftop على الخادم أثناء فيديو) | يدوي + Wireshark |
...
- واجهات `RouteBridge.Core`: `IControlChannel`, `ITunnelTransport`, `ITunnelSession`, `IBrowserSession`, `ISecretStore` + `SessionStateMachine` — تُنشر أولًا ليبني C الشاشات على `FakeTunnelSession` و`FakeBrowserSession`.
...
| **8** | مراجعة القسم 14 بندًا بندًا (مشتركة)؛ إصلاحات | المراجعة الأمنية للنفق (مشتركة)؛ Fuzz للإطارات؛ إصلاحات | إصلاحات؛ تجربة المستخدم النهائية | تشغيل قائمة القبول الـ 18 كاملة على staging؛ `certutil` وWireshark وiftop |
