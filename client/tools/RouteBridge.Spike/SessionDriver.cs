...
internal sealed class SessionDriver : ITunnelSessionFactory, IAsyncDisposable
{
    ...
    /// <summary>
    /// نفس تركيب <c>RouteBridge.App.Services.TunnelSessionFactory</c> (<see cref="EgressTunnelAdapter.Create"/> على
    ...
    private async Task CurlThroughProxyAsync(int proxyPort, string url, CancellationToken ct)
    {
        ...
        try
        {
            ...
            http.DefaultRequestHeaders.UserAgent.ParseAdd("RouteBridge.Spike/session");
