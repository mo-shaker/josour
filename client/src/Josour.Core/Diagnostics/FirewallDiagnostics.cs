...
/// <c>Josour.Tunnel.Diagnostics.HostDiagnostics</c> (track B, for <c>hello.diagnostics</c>) and once in
/// <c>Josour.Infrastructure.Diagnostics.HostDiagnosticsProbe</c> (track C, for the host's readiness warning) — with two
...
{
    /// <summary>The rule the installer adds (Josour.iss): <c>netsh advfirewall firewall add rule name="Josour Tunnel" …</c>.</summary>
    public const string RuleName = "Josour Tunnel";
