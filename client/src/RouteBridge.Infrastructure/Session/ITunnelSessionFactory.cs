...
/// <c>TunnelSessionOptions</c> and a real <c>RouteBridge.Tunnel.TunnelSession</c>.
/// <para>
/// The request deliberately carries only <c>RouteBridge.Core</c> types so that this assembly (and its tests) never has to
/// reference <c>RouteBridge.Tunnel</c>/<c>Egress</c>/<c>Proxy</c>: the App owns that wiring
