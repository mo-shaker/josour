...
{
    /// <summary><c>GET /healthz</c> answered as a RouteBridge server.</summary>
    Ok,
    ...
    /// <summary>Something answered, but not a RouteBridge health endpoint (wrong host, a captive portal, a proxy error page).</summary>
    NotRouteBridge,
}
    ...
    public async Task<ServerCheckResult> CheckAsync(string? url, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                return new ServerCheckResult(ServerCheckStatus.NotRouteBridge, null, $"HTTP {(int)response.StatusCode}");
            }

            var body = await response.Content.ReadAsStringAsync(budget.Token).ConfigureAwait(false);
            if (!LooksHealthy(body))
            {
                _logger.LogInformation("Server check {Url}: 200 but not a RouteBridge health response", health);
                return new ServerCheckResult(ServerCheckStatus.NotRouteBridge, null, "unexpected body");
            }
