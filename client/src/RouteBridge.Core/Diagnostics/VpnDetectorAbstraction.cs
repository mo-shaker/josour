...
/// (<see cref="VpnDetector"/> in <c>RouteBridge.Core.Net</c>); this interface exists only so that the layers that ACT on the
...
/// <c>RouteBridge.Tunnel.Diagnostics.HostDiagnostics.DetectVpn()</c>, which is this same detector plus that layer's own
