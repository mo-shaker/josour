...
/// One file per key under <c>%LOCALAPPDATA%\Josour\secrets\&lt;key&gt;.bin</c>.
/// <para>
...
public sealed partial class DpapiSecretStore : ISecretStore
{
    ...
    // App-specific optional entropy: ties the blob to Josour so another DPAPI consumer running as the same
    // user cannot trivially unprotect it without knowing this constant.
    private static readonly byte[] Entropy = SHA256.HashData("Josour.SecretStore.v1"u8.ToArray());
