...
internal sealed class RawMuxPeer : IAsyncDisposable
{
    ...
    public static async Task<RawMuxPeer> CreateAsync(TunnelRole victimRole = TunnelRole.Host, MuxOptions? victimOptions = null)
    {
        ...
        var options = new MultiplexingStream.Options
        {
            ...
            StartSuspended = true,
            TraceSource = new TraceSource("RouteBridge.Fuzz", SourceLevels.Off),
        };
...
///
/// <para>يُصنَّف المُلتقَط إلى ما نصنعه نحن (<see cref="IsOurs"/>: نوع في فضاء أسماء RouteBridge) وما تصنعه
...
internal sealed class UnobservedExceptionWatch : IDisposable
{
    ...
    /// <summary>هل الاستثناء من صنعنا (نوع في RouteBridge) لا من مهام المكتبة الداخلية؟</summary>
    public static bool IsOurs(Exception e)
        => e.GetType().Namespace?.StartsWith("RouteBridge", StringComparison.Ordinal) == true;
