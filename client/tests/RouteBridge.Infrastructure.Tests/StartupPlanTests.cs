...
public sealed class StartupPlanTests
{
    ...
    {
        var plan = StartupPlanner.Decide(With("https://routebridge.example.com"), credentialsStored: true, sessionRestored: false);

        Assert.Equal(StartupDestination.SignIn, plan.Destination);
        Assert.False(plan.IsFreshInstall);
        Assert.Equal("https://routebridge.example.com", plan.ServerUrl);
    }
    ...
    public void AUserWhoSignedOut_IsNotSentThroughTheFirstRunAgain()
    {
        ...
        var plan = StartupPlanner.Decide(With("https://routebridge.example.com"), credentialsStored: false, sessionRestored: false);
    ...
    {
        var plan = StartupPlanner.Decide(With("https://routebridge.example.com"), credentialsStored: true, sessionRestored: true);
    ...
    [InlineData("http://routebridge.example.com")] // plain http to a remote host is refused everywhere else too
