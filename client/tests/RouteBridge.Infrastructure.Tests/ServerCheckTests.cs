...
public sealed class ServerCheckTests
{
    ...
    [Theory]
    [InlineData("https://routebridge.example.com", "https://routebridge.example.com")]
    [InlineData("  https://RouteBridge.Example.com/  ", "https://routebridge.example.com")]
    [InlineData("https://routebridge.example.com/rb", "https://routebridge.example.com/rb")]
    ...
    [InlineData("")]
    [InlineData("   ")]
    [InlineData("routebridge.example.com")]
    [InlineData("ftp://routebridge.example.com")]
    [InlineData("https://routebridge.example.com/?x=1")]
    [InlineData("https://routebridge.example.com/#frag")]
    ...
    public void PlainHttpToARemoteHost_IsRefusedAsInsecure_NotMerelyInvalid()
    {
        ...
        var result = HttpServerCheck.Validate("http://routebridge.example.com");
    ...
    public async Task AHealthyServer_IsOk_AndTheCheckHitsHealthz()
    {
        ...
        var result = await check.CheckAsync("https://routebridge.example.com", None);

        Assert.True(result.IsOk);
        Assert.Equal("https://routebridge.example.com", result.NormalizedUrl);
    ...
    public async Task APathPrefix_IsKept_SoAServerBehindAReverseProxyStillAnswers()
    {
        ...
        var result = await check.CheckAsync("https://routebridge.example.com/rb", None);
    ...
    [Fact]
    public async Task AServerThatAnswersSomethingElse_IsNotRouteBridge()
    {
        ...
        Assert.Equal(ServerCheckStatus.NotRouteBridge, result.Status);
    }

    [Fact]
    public async Task AnHttpErrorStatus_IsAlsoNotRouteBridge()
    {
        var (check, http) = Create();
    ...
    public async Task TheCallersOwnCancellation_IsNotSwallowedAsAnUnreachableServer()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => check.CheckAsync("https://routebridge.example.com", cts.Token));
    }
}
