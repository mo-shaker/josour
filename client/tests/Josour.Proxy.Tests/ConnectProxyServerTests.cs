...
public class ConnectProxyServerTests
{
    ...
    [Theory]
    [InlineData("GET http://check.josour/ HTTP/1.1\r\nHost: check.josour\r\n\r\n")]
    [InlineData("GET http://check.josour/any/path?x=1 HTTP/1.1\r\nHost: check.josour\r\nProxy-Connection: keep-alive\r\n\r\n")]
    ...
    public async Task OwnerPid_NotOwnedByBrowser_ClosedWithoutResponse()
    {
        ...
        {
            await using var client = await ProxyClient.ConnectAsync(proxy.Port);
            try { await client.SendAsync("GET http://check.josour/ HTTP/1.1\r\n\r\n"); }
            catch (IOException) { }
