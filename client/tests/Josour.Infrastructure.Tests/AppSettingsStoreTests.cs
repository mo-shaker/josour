...
public sealed class AppSettingsStoreTests : IDisposable
{
    ...
    {
        var store = new AppSettingsStore(FilePath);
        await store.SaveAsync(new AppSettings { ServerUrl = "https://josour.example.com", Language = "en" }, CancellationToken.None);
        ...
        await File.WriteAllTextAsync(FilePath, """{"serverUrl":"https://josour.example.com","startMinimized":true}""");
    ...
    public async Task RoundTrip_ThroughDisk_AndChangedEvent()
    {
        ...
        var settings = new AppSettings { ServerUrl = "https://josour.example.com", PreferredBrowser = BrowserPreference.Edge, StartMinimized = true };
        ...
        Assert.Contains("\"serverUrl\": \"https://josour.example.com\"", text);
    ...
    public async Task TheSettingsWindowsWholeUpdate_RoundTripsInOneWrite()
    {
        ...
        await store.SaveAsync(
            store.Current with
            {
                ServerUrl = "https://josour.example.com",
                Language = "en",
        ...
        Assert.Equal("https://josour.example.com", reloaded.ServerUrl);
