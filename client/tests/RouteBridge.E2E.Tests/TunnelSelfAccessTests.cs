...
public class TunnelSelfAccessTests : IAsyncLifetime
{
    ...
    public async Task TheTunnelItselfIsStillAlive_AfterEveryRejection()
    {
        ...
        Assert.Equal(RouteBridge.Core.Tunnel.TunnelState.Connected, _pair.Host.State);
        Assert.Equal(RouteBridge.Core.Tunnel.TunnelState.Connected, _pair.Guest.State);
