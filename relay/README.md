...
## ما ينقص لتعمل الجلسة عبر الـ Relay

الخدمة جاهزة ومختبَرة، لكنها لا تُستدعى بعد. الباقي:

1. **الخادم الخلفي:** إعداد `RELAY_SECRET` وعنوان الـ Relay، وإصدار توكن لكل طرف عند إنشاء الجلسة.
2. **عقد WebSocket:** حقل `relay` في `session.created` (العنوان + التوكن)، وتوسيع مفردات
   `winner_type` بـ `relay` — و`docs/ws-protocol.md` ينبّه إلى هذا التوسعة صراحةً.
3. **العميل:** بناء `RelayTransport` من ذلك الحقل في `TunnelSessionFactory`، وتشغيله بالتوازي مع
   المباشر بحيث يفوز الأسرع.
